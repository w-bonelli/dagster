...
def get_schedule_change_set(schedule_states, external_schedules):
    check.list_param(schedule_states, 'schdule_states', ScheduleState)
    ...
    schedule_states_dict = {s.schedule_id: s for s in schedule_states}

    external_schedule_ids = set(external_schedules_dict.keys())
    schedule_state_ids = set(schedule_states_dict.keys())

    added_schedules = external_schedule_ids - schedule_state_ids
    changed_schedules = external_schedule_ids & schedule_state_ids
    removed_schedules = schedule_state_ids - external_schedule_ids

    changeset = []

    for schedule_id in added_schedules:
        changeset.append(("add", external_schedules_dict[schedule_id].name, schedule_id, []))

    for schedule_id in changed_schedules:
        changes = []

        schedule_state = schedule_states_dict[schedule_id]
        external_schedule = external_schedules_dict[schedule_id]
        ...
        if len(changes) > 0:
            changeset.append(
                ("change", external_schedules_dict[schedule_id].name, schedule_id, changes,)
            )

    for schedule_id in removed_schedules:
        changeset.append(("remove", schedule_states_dict[schedule_id].name, schedule_id, []))
...
class Scheduler(six.with_metaclass(abc.ABCMeta)):
    ...
    def _get_schedule_state(self, instance, schedule_id):
        schedule_state = instance.get_schedule_state(schedule_id)
    ...
    def reconcile_scheduler_state(self, instance, external_repository):
        ...
        schedule_ids_to_delete = existing_schedule_ids - external_schedule_ids
        ...
        for schedule_id in schedule_ids_to_delete:
            try:
                instance.stop_schedule_and_delete_from_storage(schedule_id)
            except DagsterSchedulerError as e:
                schedule_reconciliation_errors.append(e)
    ...
    def stop_schedule_and_update_storage_state(self, instance, schedule_id):
        '''
        ...
        Args:
            schedule_id (string): The id of the schedule target to stop running.
        '''

        check.str_param(schedule_id, 'schedule_id')

        schedule_state = self._get_schedule_state(instance, schedule_id)

        self.stop_schedule(instance, schedule_id)
    ...
    def stop_schedule_and_delete_from_storage(self, instance, schedule_id):
        '''
        ...
        Args:
            instance (DagsterInstance): The current instance.
            schedule_id (string): The id of the schedule target to start running.
        '''

        check.inst_param(instance, 'instance', DagsterInstance)
        check.str_param(schedule_id, 'schedule_id')

        schedule = self._get_schedule_state(instance, schedule_id)
        self.stop_schedule(instance, schedule_id)
        instance.delete_schedule_state(schedule_id)
        return schedule
    ...
    @abc.abstractmethod
    def stop_schedule(self, instance, schedule_id):
    ...
    @abc.abstractmethod
    def running_schedule_count(self, schedule_id):
        ...
        Args:
            schedule_id (string): The id of the schedule target to return the number of jobs for
        '''

    @abc.abstractmethod
    def get_logs_path(self, instance, schedule_id):
        '''Get path to store logs for schedule

        Args:
            schedule_id (string): The id of the schedule target to retrieve the log path for
        '''
...
class ScheduleTickData(
    namedtuple('Schedule', 'schedule_id schedule_name cron_schedule timestamp status run_id error')
):
    def __new__(
        cls, schedule_id, schedule_name, cron_schedule, timestamp, status, run_id=None, error=None,
    ):
        '''
        ...
        Arguments:
            schedule_id (str): The id of the schedule target for this tick
...
class ScheduleTick(namedtuple('Schedule', 'tick_id schedule_tick_data')):
    ...
    @property
    def schedule_id(self):
        return self.schedule_tick_data.schedule_id
