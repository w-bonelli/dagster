...
class SqlScheduleStorage(ScheduleStorage):
    ...
    def all_stored_schedule_state(self, repository_id=None):
        ...
        if repository_id:
            query = base_query.where(ScheduleTable.c.repository_id == repository_id)
        else:
            query = base_query
    ...
    def get_schedule_state(self, schedule_id):
        ...
        query = (
            ...
            .where(ScheduleTable.c.schedule_id == schedule_id)
        )
    ...
    def get_schedule_ticks(self, schedule_id):
        ...
        query = (
            ...
            .where(ScheduleTickTable.c.schedule_id == schedule_id)
        ...
        with self.connect() as conn:
            try:
                tick_insert = ScheduleTickTable.insert().values(  # pylint: disable=no-value-for-parameter
                    schedule_id=schedule_tick_data.schedule_id,
            ...
            try:
                ...
                six.raise_from(
                    DagsterInvariantViolationError(
                        ...
                        'in storage'.format(id=schedule.schedule_id,)
                    ),
                    exc,
                )
    ...
    def update_schedule_state(self, schedule):
        check.inst_param(schedule, 'schedule', ScheduleState)
        if not self.get_schedule_state(schedule.schedule_id):
            raise DagsterInvariantViolationError(
                'ScheduleState {id} is not present in storage'.format(id=schedule.schedule_id)
            )

        with self.connect() as conn:
            conn.execute(
                ScheduleTable.update()  # pylint: disable=no-value-for-parameter
                .where(ScheduleTable.c.schedule_id == schedule.schedule_id)
                .values(
                    status=schedule.status.value,
                    schedule_body=serialize_dagster_namedtuple(schedule),
                )
    ...
    def delete_schedule_state(self, schedule_id):
        ...
        if not self.get_schedule_state(schedule_id):
            raise DagsterInvariantViolationError(
                'ScheduleState {id} is not present in storage'.format(id=schedule_id)
            )

        with self.connect() as conn:
            conn.execute(
                ScheduleTable.delete().where(  # pylint: disable=no-value-for-parameter
                    ScheduleTable.c.schedule_id == schedule_id
                )
            )
