...
def print_changes(repository, instance, print_fn=print, preview=False):
    ...
    for change in changeset:
        change_type, schedule_name, schedule_id, changes = change

        if change_type == "add":
            print_fn(
                click.style(
                    '  + {name} (add) [{id}]'.format(name=schedule_name, id=schedule_id), fg='green'
                )
            )
        ...
        if change_type == "change":
            print_fn(
                click.style(
                    '  ~ {name} (update) [{id}]'.format(name=schedule_name, id=schedule_id),
                    fg='yellow',
                )
            )
        ...
        if change_type == "remove":
            print_fn(
                click.style(
                    '  - {name} (delete) [{id}]'.format(name=schedule_name, id=schedule_id),
                    fg='red',
                )
            )
        ...
        for schedule_state in instance.all_stored_schedule_state(
            ...
            if schedule_state.status == ScheduleStatus.RUNNING:
                try:
                    ...
                    instance.stop_schedule_and_update_storage_state(schedule_state.schedule_id)
                    instance.start_schedule_and_update_storage_state(external_schedule)
                except DagsterInvariantViolationError as ex:
                    raise click.UsageError(ex)
