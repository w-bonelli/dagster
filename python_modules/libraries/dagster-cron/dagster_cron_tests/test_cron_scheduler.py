...
def test_start_and_stop_schedule(
    ...
    with TemporaryDirectory() as tempdir:
        ...
        schedule_id = schedule.get_reconstruction_id()

        instance.start_schedule_and_update_storage_state(schedule)

        assert 'schedules' in os.listdir(tempdir)

        assert "{}.sh".format(schedule_id) in os.listdir(
            os.path.join(tempdir, 'schedules', 'scripts')
        )

        instance.stop_schedule_and_update_storage_state(schedule_id)

        assert "{}.sh".format(schedule_id) not in os.listdir(
            os.path.join(tempdir, 'schedules', 'scripts')
        )
...
def test_start_schedule_cron_job(
    ...
    with TemporaryDirectory() as tempdir:
        ...
        for cron_job in cron_jobs:
            ...
            cron_schedule, command, log_file, schedule_id = match[0]

            schedule_def = external_schedules_dict[schedule_id]
...
def test_script_execution(
    ...
    with TemporaryDirectory() as tempdir:
        ...
        schedule_id = external_repo.get_external_schedule(
            "no_config_pipeline_every_min_schedule"
        ).get_reconstruction_id()
...
def test_stop_schedule_fails(
    ...
    with TemporaryDirectory() as tempdir:
        ...
        schedule = instance.get_schedule_state(schedule_id)
...
def test_log_directory(restore_cron_tab):  # pylint:disable=unused-argument,redefined-outer-name
    with TemporaryDirectory() as tempdir:
        ...
        assert schedule_log_path.endswith(
            "/schedules/logs/{schedule_id}/scheduler.log".format(
                schedule_id=external_schedule.get_reconstruction_id()
            )
        )
