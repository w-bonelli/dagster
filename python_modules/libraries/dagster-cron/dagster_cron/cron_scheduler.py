...
class SystemCronScheduler(Scheduler, ConfigurableClass):
    ...
    def start_schedule(self, instance, external_schedule):
        ...
        schedule_id = external_schedule.get_reconstruction_id()
        ...
        if self.running_schedule_count(external_schedule.get_reconstruction_id()) > 0:
            self._end_cron_job(instance, schedule_id)

        self._start_cron_job(instance, external_schedule)

        # Verify that the cron job is running
        running_schedule_count = self.running_schedule_count(schedule_id)
    ...
    def _get_bash_script_file_path(self, instance, schedule_id):
        check.inst_param(instance, 'instance', DagsterInstance)
        check.str_param(schedule_id, 'schedule_id')
        ...
        script_file_name = "{}.sh".format(schedule_id)
        return os.path.join(script_directory, script_file_name)

    def _cron_tag_for_schedule(self, schedule_id):
        return 'dagster-schedule: {schedule_id}'.format(schedule_id=schedule_id)

    def _get_command(self, script_file, instance, schedule_id):
        schedule_log_file_path = self.get_logs_path(instance, schedule_id)
    ...
    def _start_cron_job(self, instance, external_schedule):
        ...
        command = self._get_command(script_file, instance, schedule_id)

        job = self._cron_tab.new(
            command=command,
            comment='dagster-schedule: {schedule_id}'.format(schedule_id=schedule_id),
        )
        job.setall(external_schedule.cron_schedule)
        self._cron_tab.write()

    def _end_cron_job(self, instance, schedule_id):
        self._cron_tab.remove_all(comment=self._cron_tag_for_schedule(schedule_id))
        self._cron_tab.write()

        script_file = self._get_bash_script_file_path(instance, schedule_id)
        if os.path.isfile(script_file):
            os.remove(script_file)

    def running_schedule_count(self, schedule_id):
        cron_tab = CronTab(user=True)
        matching_jobs = cron_tab.find_comment(self._cron_tag_for_schedule(schedule_id))

        return len(list(matching_jobs))

    def _get_or_create_logs_directory(self, instance, schedule_id):
        check.inst_param(instance, 'instance', DagsterInstance)
        check.str_param(schedule_id, 'schedule_id')

        logs_directory = os.path.join(instance.schedules_directory(), "logs", schedule_id)
    ...
    def get_logs_path(self, instance, schedule_id):
        ...
        logs_directory = self._get_or_create_logs_directory(instance, schedule_id)
    ...
    def _write_bash_script_to_file(self, instance, external_schedule):
        ...
        logs_directory = self._get_or_create_logs_directory(instance, schedule_id)
        schedule_log_file_name = "{}_{}.result".format("${RUN_DATE}", schedule_id)
